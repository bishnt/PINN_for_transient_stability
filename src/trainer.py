...
class PINNTrainer:
    ...
    def _compute_adaptive_weights(self, batch: Dict):
        ...
        # Normalise so weights sum to 3 (preserve scale)
    ...
    def train_adam(
        ...
        lr: float = 1e-3,
        log_every: int = 500,
        rebalance_every: int = 1000,   # how often to recompute weights
    ):
        ...
        for epoch in pbar:
            ...
            optimizer.step()
            scheduler.step(total.detach())      # ← fixes the UserWarning too
    ...
    def train_lbfgs(                            # ← correctly indented in class
        self,
        batch: Dict,
        n_epochs: int = 400,
        lr: float = 0.05,
